...
public class NetworkTopology {
	...
	/**
	 * This method return cloudsimEntityId given BriteID
	 * @return CloudsimEntityID
	 * @param BID
	 */
	public static int EntityId(int BID) {
		for (int key: map.keySet()){
			if(map.get(key).equals(BID)){
				return key;
			}
		}
		return -1;
	...
	/**
	 * This method printOut topologyGraph
	 * @return $string graph info
	 */
	public static String printOutGraph() {
		StringBuffer buffer = new StringBuffer();
		Iterator<TopologicalNode> nodeList = graph.getNodeIterator();
		buffer.append("topological-node-information: ");
		while(nodeList.hasNext()){
			
			buffer.append(System.lineSeparator());
			TopologicalNode node = nodeList.next();
				buffer.append("BriteID:" + node.getNodeID() + "  EntityId:" + EntityId(node.getNodeID()) +" | x is: " + node.getCoordinateX() + " y is: "
						+ node.getCoordinateY() );
			}
